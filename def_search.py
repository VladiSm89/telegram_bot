...
def processing_of_data_for_printing(result):
...
# Convert None to "нет данных" and float to int
    ...
    info_adres = dict(zip(keys, values))

    non_output_list = tuple() # дабвляем ключи елементов словаря info_adres которых не должно быть в выводе

    result = '\n'.join([f'{key}: {value}' for key, value in info_adres.items() if key not in non_output_list
